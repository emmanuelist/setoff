...
export type Attempt = {
  key: string;
  group: "The fixing" | "Paying" | "Authority" | "Recording";
  title: string;
  detail: string;
...
export async function runAttempt(attempt: Attempt, c: RoomContext): Promise<Outcome> {
  ...
  try {
    ...
    (decoded.args ?? []).forEach((v, i) => {
      ...
      args[input?.name || `arg${i}`] = input?.type === "bytes3" ? decodeCurrency(v as Hex) : input?.name === "state" ? ["None", "Proposed", "Accepted", "Paid", "Cancelled"][Number(v)] ?? show(v) : show(v);
    });
