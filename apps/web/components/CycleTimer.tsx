...
export function CycleTimer({ openedAt, cutoff, deadline, fixedAt, closedAt, state, readAt }: Props) {
  ...
  const stage =
    state === "settled" ? { word: "Settled", line: `at ${clock(closedAt ?? 0)}. Every net debtor funded, so every debt was netted at once.` }
    : state === "void" ? { word: "Voided", line: `at ${clock(closedAt ?? 0)}. Someone didn't fund, so every deposit became refundable.` }
  ...
  return (
    <div className="grid gap-4">
      <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
        <span className="text-[17px] font-semibold">{stage.word}</span>
        <span className="text-[13px] text-graphite">{stage.line}</span>
      </div>
      <div className="relative pt-1 pb-7" aria-hidden="true">
        ...
        {[{ t: cutoff, label: `Cutoff ${clock(cutoff)}` }, { t: deadline, label: `Deadline ${clock(deadline)}` }].map((m, i) => (
          <div key={m.label} className="absolute top-0 flex flex-col items-center" style={{ left: `${pos(m.t)}%`, transform: `translateX(${i === 1 ? "-100%" : "-50%"})` }}>
            ...
            <span className={`fig mt-1 text-[11px] whitespace-nowrap text-graphite ${i === 1 ? "self-end" : ""}`}>{m.label}</span>
          </div>
        ))}
        {fixedAt !== null && <span className="absolute top-1 h-3.5 w-0.5 bg-endorse" style={{ left: `${pos(fixedAt)}%` }} title={`Fixed ${clock(fixedAt)}`} />}
        <span
      ...
      <p className="sr">{stage.word}. {stage.line} Cutoff {clock(cutoff)}, funding deadline {clock(deadline)}.</p>
    </div>
  );
}
