  ...
  return (
    <div ref={root} className="grid gap-7">
      <div className="flex flex-wrap items-end justify-between gap-x-8 gap-y-4">
        ...
        <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
          <NumberFlow
            ...
            className={`fig leading-none font-medium tracking-[-0.02em] ${compact ? "text-[clamp(40px,5vw,60px)]" : "text-[clamp(48px,7vw,96px)]"}`}
            aria-hidden="true"
          />
          <span className="text-[15px] leading-tight text-graphite sm:text-[17px]" aria-hidden="true">
        ...
        <div className="grid justify-items-start gap-2 text-[13px] text-graphite sm:justify-items-end sm:text-right">
      ...
      <div className="well grid gap-1 p-3 sm:p-4" aria-hidden="true">
        <div className="grid grid-cols-[minmax(0,1fr)] gap-x-4 pb-1 text-[11px] sm:grid-cols-[108px_minmax(0,1fr)_132px]">
        ...
        {rows.map((r) => {
          ...
          return (
            <div key={r.party} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-4 gap-y-1.5 border-t border-dashed border-ink/15 py-2.5 first:border-0 sm:grid-cols-[108px_minmax(0,1fr)_132px]">
              <span className="fig text-[12px] text-graphite">{short(r.party)}</span>
              <div className="relative col-span-2 row-start-2 h-6 sm:col-span-1 sm:row-start-auto">
                ...
                <Beam side="owes" segments={r.owes} max={max} setOff={setOff} net={net < 0n ? -net : 0n} reduce={!!reduce} />
                <Beam side="owed" segments={r.owed} max={max} setOff={setOff} net={net > 0n ? net : 0n} reduce={!!reduce} />
              </div>
              <span className="text-right">
                <span className={`fig block text-[13px] transition-opacity duration-500 ${setOff ? "opacity-100" : "opacity-40"}`}>{net > 0n ? "+" : net < 0n ? "−" : ""}{formatUsdc(net < 0n ? -net : net, 4)}</span>
                <span className="legend text-[9.5px]">
                  {word}
      ...
      {compact && (
        ...
        <p className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[12px] text-graphite" aria-hidden="true">
          {currencies.map((c) => (
            <span key={c} className={`ccy-${c.toLowerCase()} inline-flex items-center gap-1.5`}>
              ...
              <span className="code text-[10.5px]">{c}</span>
            </span>
          ))}
      ...
      {!compact && (
        <ul className="flex flex-wrap gap-2" aria-label="Gross owed in each currency">
          {currencies.map((c) => {
            ...
            return (
              <li key={c} className={`stock ccy-${c.toLowerCase()} flex items-baseline gap-2 px-3 py-2 text-[13px]`}>
                <span className="code text-[11px]">{c}</span>
                <span className="fig">{formatAmount(amount, c)}</span>
                <span className="text-graphite">→</span>
                <span className="fig">{formatUsdc(usdc, 4)}</span>
                <span className="text-[11.5px] text-graphite">USDC</span>
              </li>
            );
          })}
        </ul>
...
/** One half of a party's beam: the gross in currency stock, ghosting back as the net grows in ink. */
function Beam({ side, segments, max, setOff, net, reduce }: { side: "owes" | "owed"; segments: Side; max: bigint; setOff: boolean; net: bigint; reduce: boolean }) {
  ...
  return (
    <div className={`absolute inset-y-0 w-1/2 ${left ? "right-1/2" : "left-1/2"}`}>
      <motion.div
        className={`flex h-full ${left ? "flex-row-reverse" : "flex-row"}`}
        animate={{ opacity: setOff ? 0.5 : 1 }}
      ...
      >
        {segments.map((s, i) => (
          <span
            ...
            style={{ width: `${pct(s.usdc, max)}%`, minWidth: s.usdc > 0n ? 2 : 0 }}
          />
        ))}
      </motion.div>
      <motion.span
        ...
        transition={{ duration: reduce ? 0 : 1.4, ease, delay: reduce ? 0 : 0.35 }}
      />
    </div>
  );
}
