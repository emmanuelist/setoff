  ...
  return (
    <div className="grid gap-7">
      <div className="grid gap-2" role="group" aria-labelledby="lead-label">
        ...
        <span className="text-[12.5px] text-graphite">Until then, creditors bill into the cycle and debtors endorse. At the cutoff the list is closed.</span>
      </div>
      ...
      <div className="grid gap-2" role="group" aria-labelledby="window-label">
        ...
        <span className="text-[12.5px] text-graphite">Anyone can fix the cycle from the cutoff until the deadline. If anyone hasn&apos;t funded by then, the cycle is voided and every deposit comes back.</span>
      </div>

      <dl className="well grid gap-2 px-4 py-3.5 text-[13px] sm:grid-cols-2">
        <div className="grid gap-0.5"><dt className="legend">Cutoff</dt><dd className="fig">{clock(cutoff)}</dd><dd className="text-[12px] text-graphite">about {LEADS.find((l) => l.v === lead)?.label} after you sign</dd></div>
        <div className="grid gap-0.5"><dt className="legend">Funding deadline</dt><dd className="fig">{clock(deadline)}</dd><dd className="text-[12px] text-graphite">{WINDOWS.find((w) => w.v === window_)?.label} after the cutoff</dd></div>
      </dl>
