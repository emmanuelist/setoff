...
import { pad, stamp, utc } from "@/lib/format";
import { formatUsdc, roundDecimal } from "@/lib/money";
import { setoffAbi } from "@/lib/setoff-abi";
import { readCycles, readMaxFixingAge } from "@/lib/setoff";
...
export default async function Cycles() {
  await connection();
  const [cycles, head, maxAge] = await Promise.all([readCycles(), publicClient.getBlock(), readMaxFixingAge()]);
    ...
    <main className="px-[var(--gutter)] pt-[var(--seam)]">
      <div className="bento">
        <section className="plate col-span-12 grid gap-6 p-6 sm:p-9 lg:col-span-7">
          <h1 className="max-w-[16ch] text-display leading-[1.0] font-bold tracking-[-0.022em] [font-variation-settings:'wdth'_108]">Only the net moves.</h1>
        ...
        <Plate legend="What a cycle can't do" className="col-span-12 lg:col-span-5">
          <ul className="grid gap-3 text-body leading-[1.45]">
            ...
            <li>Price a debt at a stale rate. A fixing older than <span className="fig">{Math.round(maxAge / 3600)} h</span> is refused.</li>
          </ul>
        </Plate>
        ...
        <Plate legend="Every cycle" aside={<span><span className="fig text-ink">{cycles.length}</span> on the contract, newest first</span>} className="col-span-12">
          {cycles.length === 0 ? (
            <div className="well grid justify-items-start gap-4 p-6">
              <p className="text-body text-graphite">No cycle has been opened yet. Anyone can open one; debts join it when their debtors endorse them.</p>
            ...
            <div className="well grid gap-2 p-2">
              {cycles.map((c, i) => {
                ...
                const when = c.state === "open" ? (now < c.cutoff ? `cutoff ${stamp(c.cutoff, now)}` : `fixable until ${stamp(c.fundingDeadline, now)}`)
                  : c.state === "fixed" ? `funding until ${stamp(c.fundingDeadline, now)}` : utc(c.closedAt ?? 0);
                return (
                  <Link key={c.id.toString()} href={`/cycles/${c.id}`} className="stock grid grid-cols-[1fr_auto] items-center gap-x-4 gap-y-2 px-4 py-3.5 no-underline transition-[transform,box-shadow] duration-200 ease-spring hover:-translate-y-[3px] md:grid-cols-[120px_120px_minmax(0,1fr)_minmax(0,1fr)_auto]">
                    <span className="text-caption"><ContractBand label={pad(c.id, 4)} /></span>
                    <span className="justify-self-end md:justify-self-start"><CycleMark state={c.state} size="sm" /></span>
                    <span className="text-small text-graphite"><span className="fig text-ink">{counts[i]}</span> {counts[i] === 1 ? "debt" : "debts"} · <span className="fig">{when}</span></span>
